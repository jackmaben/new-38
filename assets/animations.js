...
(function () {
  ...
  function measureRail() {
    ...
    // Measure against .steps with rects (offsetTop depends on offsetParent, which is
    // wrong before layout settles) and clamp so the rail can never overrun the list.
    const sr = steps.getBoundingClientRect(), lr = last.getBoundingClientRect();
    railLen = Math.max(0, Math.min(sr.height, (lr.top - sr.top) + dotTop + 8) - 10);
  ...
  // Pause decorative loops when the tab is not visible (saves battery + main thread)
  document.addEventListener('visibilitychange', () => {
    document.body.classList.toggle('perf-idle', document.hidden);
  });
    ...
    Array.from(h1.childNodes).forEach(node => {
      if (node.nodeType === 3) {
        ...
        node.textContent.split(/(\s+)/).forEach(tok => {
          ...
          w.className = 'word';
          w.textContent = tok;
          w.style.setProperty('--wd', (0.02 + idx * 0.028).toFixed(3) + 's');
          idx++;
          frag.appendChild(w);
        });
