...
(function () {
  ...
  /* ---------- Device capability gate ----------
     Weak devices pay the most for blurred layers + the particle canvas, so
     they get a lighter page instead of a janky one. */
  var LITE = (function () {
    var nav = window.navigator || {};
    var slowCpu = (nav.hardwareConcurrency || 8) <= 4;
    ...
    return !!(slowCpu || lowMem || save || small);
  })();
  if (LITE) document.documentElement.classList.add('perf-lite');

  /* ---------- FAQ click ripple ---------- */
  function initFaqRipple() {
    document.querySelectorAll('details.faq-item > summary').forEach(function (s) {
      if (s.dataset.rippleBound) return;
      s.dataset.rippleBound = '1';
      ...
      s.addEventListener('animationend', function () { s.classList.remove('pressed'); });
    });
  }
  ...
  function initAurora() {
    ...
    a.innerHTML = '<b></b><b></b><b></b><b></b><b></b><b></b><i class="sheen"></i>';
    document.body.insertBefore(a, document.body.firstChild);
    /* Depth: aurora drifts slower than the page, and leans toward the cursor. */
    var mx = 0, my = 0, sy = 0, raf = 0;
    function paint() {
      raf = 0;
      a.style.setProperty('--px', mx.toFixed(2) + 'px');
      a.style.setProperty('--py', (my + sy).toFixed(2) + 'px');
    }
    function queue() { if (!raf) raf = requestAnimationFrame(paint); }
    window.addEventListener('pointermove', function (e) {
      mx = (e.clientX / window.innerWidth - .5) * 44;
  ...
  /* ---------- New section structures: reveals, tabs, counters, rail draw ---------- */
  function initStructures() {
    var reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    var hasIO = 'IntersectionObserver' in window;

    // Tabs — sliding indicator, keyboard nav, colour follows the active tab
    document.querySelectorAll('[data-tabset]').forEach(function (set) {
      var rail = set.querySelector('.tab-rail');
      var tabs = [].slice.call(set.querySelectorAll('.tab'));
      var panels = [].slice.call(set.querySelectorAll('.tab-panel'));
      if (!tabs.length || !rail) return;
      ...
      function place(i) {
        ...
        var c = tab.style.getPropertyValue('--tc') || '#2563eb';
        ind.style.setProperty('--tc', c);
        set.style.setProperty('--ta', c);
      }
      function select(i) {
        tabs.forEach(function (x, k) { x.classList.toggle('on', k === i); x.setAttribute('aria-selected', k === i); x.setAttribute('tabindex', k === i ? '0' : '-1'); });
        panels.forEach(function (p, k) {
          if (k === i) { p.classList.remove('on'); void p.offsetWidth; p.classList.add('on'); }
      ...
      function reveal(i) {
        var tab = tabs[i];
        if (!tab || rail.scrollWidth <= rail.clientWidth + 1) return;
        var target = tab.offsetLeft - (rail.clientWidth - tab.offsetWidth) / 2;
      ...
      var active = tabs.findIndex(function (x) { return x.classList.contains('on'); });
      place(active < 0 ? 0 : active);
      window.addEventListener('resize', function () {
        var a = tabs.findIndex(function (x) { return x.classList.contains('on'); });
        place(a < 0 ? 0 : a);
      });
      if (document.fonts && document.fonts.ready) document.fonts.ready.then(function () { place(tabs.findIndex(function (x) { return x.classList.contains('on'); }) || 0); });
    });

    // Staggered reveals
    var groups = [['.hex', 70], ['.dip-row', 80], ['.ros-card', 80], ['.tile', 55], ['.layer', 70], ['.dsc', 60], ['.cb-item', 70], ['.ix', 75], ['.pil', 90]];
    groups.forEach(function (g) {
      var els = document.querySelectorAll(g[0]);
      if (!els.length) return;
      if (!hasIO || reduce) { [].forEach.call(els, function (e) { e.classList.add('in'); }); return; }
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) {
          if (!e.isIntersecting) return;
          var el = e.target;
          var i = [].indexOf.call(el.parentNode.children, el);
          setTimeout(function () { el.classList.add('in'); }, (i % 6) * g[1]);
          io.unobserve(el);
        });
    ...
    // Gate table: stagger rows, then fill the stage bars
    document.querySelectorAll('[data-gtable]').forEach(function (g) {
      if (!hasIO || reduce) { g.classList.add('in'); return; }
      [].slice.call(g.querySelectorAll('tbody tr')).forEach(function (r, i) {
        r.style.transitionDelay = (0.06 + i * 0.07) + 's';
        var bar = r.querySelector('.gt-bar i');
        if (bar) bar.style.transitionDelay = (0.2 + i * 0.09) + 's';
      });
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (e.isIntersecting) { g.classList.add('in'); io.unobserve(g); } });
      }, { threshold: 0.1 });
      io.observe(g);
    });

    // Stack board: pop the chips in sequence
    document.querySelectorAll('[data-stackboard]').forEach(function (b) {
      if (!hasIO || reduce) { b.classList.add('in'); return; }
      [].slice.call(b.querySelectorAll('.sb-chip')).forEach(function (c, i) { c.style.transitionDelay = (i * 0.045) + 's'; });
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (e.isIntersecting) { b.classList.add('in'); io.unobserve(b); } });
      }, { threshold: 0.12 });
      io.observe(b);
    });

    // Stack tower: draw the spine, then stagger the rows
    document.querySelectorAll('[data-tower]').forEach(function (tw) {
      if (!hasIO || reduce) { tw.classList.add('in'); return; }
      [].slice.call(tw.querySelectorAll('.tw-row')).forEach(function (r, i) { r.style.transitionDelay = (0.12 + i * 0.1) + 's'; });
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (e.isIntersecting) { tw.classList.add('in'); io.unobserve(tw); } });
      }, { threshold: 0.1 });
      io.observe(tw);
    });

    // Release gates: draw the spine, then stagger the rows
    document.querySelectorAll('[data-gates]').forEach(function (g) {
      if (!hasIO || reduce) { g.classList.add('in'); return; }
      [].slice.call(g.querySelectorAll('.gt-row')).forEach(function (r, i) { r.style.transitionDelay = (0.1 + i * 0.09) + 's'; });
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (e.isIntersecting) { g.classList.add('in'); io.unobserve(g); } });
      }, { threshold: 0.1 });
      io.observe(g);
    ...
    // Flow rail: draw the connector, then stagger the items
    document.querySelectorAll('[data-flowrail]').forEach(function (rail) {
      if (!hasIO || reduce) { rail.classList.add('in'); return; }
      var items = [].slice.call(rail.querySelectorAll('.fr-item'));
      items.forEach(function (it, i) { it.style.transitionDelay = (0.15 + i * 0.09) + 's'; });
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (e.isIntersecting) { rail.classList.add('in'); io.unobserve(rail); } });
      }, { threshold: 0.15 });
      io.observe(rail);
    });

    // Animated counters
    document.querySelectorAll('[data-counters]').forEach(function (band) {
      var cells = [].slice.call(band.querySelectorAll('[data-count]'));
      if (!cells.length) return;
      if (!hasIO || reduce) { cells.forEach(function (c) { c.textContent = c.getAttribute('data-count'); }); return; }
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) {
          if (!e.isIntersecting) return;
          io.unobserve(band);
  ...
  /* ---------- Capability ledger: staggered row reveal ---------- */
  function initLedger() {
    var rows = document.querySelectorAll('.led-row');
    if (!rows.length) return;
    if (!('IntersectionObserver' in window) || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      Array.prototype.forEach.call(rows, function (r) { r.classList.add('led-in'); });
      return;
    }
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        ...
        setTimeout(function () { row.classList.add('led-in'); }, (i % 6) * 70);
        io.unobserve(row);
      });
    }, { rootMargin: '0px 0px -6% 0px', threshold: 0.05 });
  ...
  /* ---------- Article: reading progress + TOC scroll-spy + content reveal ---------- */
  function initArticleScroll() {
    ...
    var links = toc ? Array.prototype.slice.call(toc.querySelectorAll('ol a[href^="#"]')) : [];
    ...
    function onScroll() {
      ...
      bar.classList.toggle('show', window.scrollY > top - 200);

      if (!links.length) return;
      var line = window.scrollY + (window.innerWidth <= 1000 ? 180 : 140), found = -1;
      for (var i = 0; i < targets.length; i++) {
        if (targets[i] && docTop(targets[i]) <= line) found = i;
      }
      if (found !== current) {
        if (links[current]) links[current].classList.remove('active');
        if (links[found]) {
          links[found].classList.add('active');
          // keep the active item in view inside whichever box actually scrolls
          var box = links[found].closest('ol');
          if (!box || box.scrollHeight <= box.clientHeight + 8) box = toc;
          if (box && box.scrollHeight > box.clientHeight + 8) {
            var r = links[found].getBoundingClientRect(), rr = box.getBoundingClientRect();
            if (r.top < rr.top + 20 || r.bottom > rr.bottom - 20) {
              box.scrollTop += (r.top - rr.top) - box.clientHeight / 2 + r.height / 2;
            }
          }
        }
        current = found;
    ...
    // Mark table wraps that actually overflow, so the scroll hint only shows when true
    var wraps = document.querySelectorAll('.doc-table-wrap');
    if (wraps.length) {
      var markWraps = function () {
      ...
      if (document.fonts && document.fonts.ready) document.fonts.ready.then(markWraps);
    }

    // 3. Reveal article blocks as they enter the viewport
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    if (!('IntersectionObserver' in window)) return;
    var blocks = article.querySelectorAll('h2, h3, p, ul, ol, table, .doc-table-wrap, .callout, blockquote, figure');
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
    ...
    Array.prototype.forEach.call(blocks, function (el) {
      if (el.closest('.mid-cta')) return;
      el.classList.add('a-rise');
      io.observe(el);
    });
  }
  ...
  function initTocToggle() {
    ...
    head.setAttribute('aria-controls', (function () {
      var ol = toc.querySelector('ol');
      if (!ol) return '';
      if (!ol.id) ol.id = 'toc-list';
      return ol.id;
    })());
    // keep the sticky offsets in sync with the real header height
    function syncHeader() {
    ...
    toc.querySelectorAll('ol a').forEach(function (a) {
      a.addEventListener('click', function () {
        if (window.innerWidth <= 1000) toc.classList.remove('open');
      });
    });
    // tapping anywhere else, or Escape, also closes it
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key !== 'Escape' || !toc.classList.contains('open')) return;
      toc.classList.remove('open');
      head.setAttribute('aria-expanded', 'false');
    });
  }
  ...
  function initFloatingCta() {
    ...
    if (!onContact) {
      ...
      // Mirror the page's own primary CTA wording (hero first, then the
      // closing section CTA) so the pill matches the page instead of the
      // generic header label.
      var src = document.querySelector('.hero-actions a.cta-btn') ||
                document.querySelector('.ready-cta-action a.cta-btn') ||
                document.querySelector('.mid-cta-actions a.cta-btn');
      var label = 'Book 30-Min Tech Session';
      if (src) {
        var t = src.textContent.replace(/\u2192/g, '').trim();
        if (t) label = t;
      }
      a.innerHTML = '<span class="fc-dot"></span> ' + label + ' <span class="fc-arrow">\u2192</span>';
  ...
  function init() {
    ...
    initSectionVariety();
    initReveal();
    // Restored navigation (back/forward, bfcache) can resume mid-page with the
    // observer already fired. Force everything visible in that case.
    window.addEventListener('pageshow', function (e) {
      if (!e.persisted && window.scrollY < 4) return;
      document.querySelectorAll('.reveal:not(.in)').forEach(function (el) { el.classList.add('in'); });
    });
    initSmoothScroll();
    initForms();
    ...
    idle(function () {
      /* INP: the aurora injects blurred layers and the ripple binds listeners —
     neither is needed for first paint, so yield the main thread first. */
  var idle = window.requestIdleCallback || function (fn) { return setTimeout(fn, 1); };
  initFaqRipple();
  idle(function () { initAurora(); }, { timeout: 900 });
